...
def load_data(smote_bool=False, pca_bool=False):
    # Load data
    ...
    if smote_bool:
        ...
        X_train_resampled, y_train = smote.fit_resample(X_train_scaled, y_train)  # Apply SMOTE

        X_train_resampled = scaler.inverse_transform(X_train_resampled)  # Inverse the scaling

        # Convert numpy arrays back to pandas DataFrame
        X_train = pd.DataFrame(X_train_resampled, columns=X_train.columns)
    else:
        # If SMOTE is not applied, simply use the split data as is
        X_train = X_train.copy()

    # Ensure test data is also a DataFrame, if it's not altered it remains as split
    X_test = X_test.copy()
